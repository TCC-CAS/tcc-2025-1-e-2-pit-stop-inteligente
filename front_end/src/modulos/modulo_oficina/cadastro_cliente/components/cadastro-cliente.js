...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function salvarCliente() {
        ...
        // Tratamento de checkboxes (se não marcado, não vem no FormData)
        dados.preferencias = {
            whatsapp: document.getElementById('pref_whatsapp').checked,
            email: document.getElementById('pref_email').checked,
            sms: document.getElementById('pref_sms').checked
        };
        ...
        lista.forEach(cliente => {
            ...
            card.innerHTML = `
                <div class="client-info">
                    <h4>${cliente.nome}</h4>
                    <p><i class="fas fa-id-card"></i> ${cliente.documento || '-'}</p>
    ...
    function carregarNoFormulario(cliente) {
        ...
        if(cliente.preferencias) {
            document.getElementById('pref_whatsapp').checked = !!cliente.preferencias.whatsapp;
            document.getElementById('pref_email').checked = !!cliente.preferencias.email;
            document.getElementById('pref_sms').checked = !!cliente.preferencias.sms;
        }

        // Highlight na lista
        document.querySelectorAll('.client-card').forEach(c => c.classList.remove('active'));
        // (Opcional) Adicionar active visualmente no item clicado se tiver ID
    }
