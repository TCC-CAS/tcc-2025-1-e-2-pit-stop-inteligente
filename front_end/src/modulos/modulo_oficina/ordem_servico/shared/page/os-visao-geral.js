...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function buscarClientes(termo, suggestionsContainer) {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/oficina/clientes/?search=${termo}`,
      );
  ...
  if (inputCliente && clienteSuggestions) {
    inputCliente.addEventListener("input", (e) =>
      handleAutocompleteInput(e, clienteSuggestions),
    );
  }

  if (inputCpfCnpj && cpfCnpjSuggestions) {
    inputCpfCnpj.addEventListener("input", (e) =>
      handleAutocompleteInput(e, cpfCnpjSuggestions),
    );
  }
  ...
  document.addEventListener("click", (e) => {
    if (
      ...
      !clienteSuggestions.contains(e.target)
    ) {
    ...
    if (
      ...
      !cpfCnpjSuggestions.contains(e.target)
    ) {
  ...
  // =========================================================
  // VARIÁVEIS DO AUTOCOMPLETE DE VEÍCULO
  // =========================================================
  ...
  async function buscarVeiculos(termo) {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/oficina/veiculos/?search=${termo}`,
      );
  ...
  function handlePlacaInput(e) {
    clearTimeout(debouncePlacaTimeout);
    const termo = e.target.value;
    debouncePlacaTimeout = setTimeout(() => {
      buscarVeiculos(termo);
    }, 500);
  }
  inputPlaca.addEventListener("input", handlePlacaInput);
  ...
  const tabModules = {
    ...
    diagnostico: () =>
      import("../../diagnostico_orcamento/components/diagnostico-tab.js"),
  ...
  async function loadTabContent(tabName) {
    ...
    const paths = {
      ...
      diagnostico:
        "../../diagnostico_orcamento/components/diagnostico-tab.html",
    ...
    try {
      ...
      if (tabModules[tabName]) {
        ...
        } else if (module.initHistorico && tabName === "historico") {
          module.initHistorico(window.osSelecionadoId);
        }
      }
    } catch (error) {
  ...
  if (tabsComponent) {
    tabsComponent.addEventListener("os:tab-change", (e) =>
      loadTabContent(e.detail.targetId),
    );
  }
    ...
    osList.addEventListener("os:create-new", () => {
      if (modalNovaOS && typeof modalNovaOS.open === "function") {
        ...
        setTimeout(() => {
          ...
          if (cpfCnpjInput) {
            cpfCnpjInput.removeEventListener(
              "input",
              window.mascaraCPFCNPJHandler,
            );
    ...
    osList.addEventListener("os:select", async (e) => {
      ...
      try {
        ...
        document.getElementById("header-veiculo").textContent =
          osDetalhes.veiculo_detalhes?.modelo || "--";
        document.getElementById("header-placa").textContent =
          osDetalhes.veiculo_detalhes?.placa || "--";
        ...
        if (tabsComponent) {
          try {
            const checklistResponse = await fetch(
              `http://127.0.0.1:8000/api/oficina/checklist/${osId}/`,
            );
            if (checklistResponse.ok) {
              const checklistData = await checklistResponse.json();
              const concluido =
                checklistData && checklistData.concluido === true;
  ...
  if (btnCriar) {
    btnCriar.addEventListener("click", async () => {
      ...
      const novaOS = {
        ...
        modelo:
          document.getElementById("modelo").value.trim() || "Não informado",
      ...
      try {
        const response = await fetch(
          "http://127.0.0.1:8000/api/oficina/os/criar/",
          {
            method: "POST",
            credentials: "include",
            headers: {
        ...
        if (response.ok) {
          ...
          let mensagemErro = "Erro ao salvar. Verifique o console do Django.";
          try {
            const erroData = await response.json();
            if (erroData.cpf_cnpj) {
              mensagemErro = "CPF/CNPJ já cadastrado para esta oficina.";
            } else if (erroData.erro) {
              mensagemErro = erroData.erro;
            } else if (erroData.detail) {
              mensagemErro = erroData.detail;
            } else {
              mensagemErro = JSON.stringify(erroData);
            }
          } catch (e) {
            const textoErro = await response.text();
            if (textoErro.includes("cpf_cnpj")) {
              mensagemErro = "CPF/CNPJ já cadastrado para esta oficina.";
            } else {
              mensagemErro = textoErro || "Erro desconhecido.";
            }
          }
          alert(mensagemErro);
        }
