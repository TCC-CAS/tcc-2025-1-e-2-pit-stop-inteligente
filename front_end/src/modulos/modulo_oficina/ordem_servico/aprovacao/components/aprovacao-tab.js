...
export function initAprovacao(osId) {
    ...
    document.getElementById('btnGerenciarAprovacao')?.addEventListener('click', abrirModalAprovacao);
}
...
async function carregarTabelaAprovacao() {
    ...
    tabelaBody.innerHTML = '<tr><td colspan="3" class="text-center">Carregando...</td></tr>';
    ...
    try {
        ...
        if (itens.length === 0) {
            tabelaBody.innerHTML = '<tr><td colspan="3" class="text-center">Nenhum item para aprovação.</td></tr>';
            return;
        }

        itens.forEach(item => {
            const statusClass = item.status_aprovacao === 'aprovado' ? 'badge-success' :
                               item.status_aprovacao === 'reprovado' ? 'badge-danger' : 'badge-warning';
        ...
        console.error(error);
        tabelaBody.innerHTML = '<tr><td colspan="3" class="text-center" style="color:red;">Erro ao carregar itens.</td></tr>';
    }
}
...
async function preencherModalAprovacao() {
    try {
        ...
        try {
            const osResponse = await fetch(`${API_BASE_URL}/os/${currentOsId}/`);
            if (osResponse.ok) {
                const osData = await osResponse.json();
                
                document.getElementById('modalStatusGeral').textContent = osData.status ? osData.status.toUpperCase() : 'PENDENTE';
                
                const dataCriacao = osData.criado_em ? new Date(osData.criado_em) : new Date();
                document.getElementById('modalDataEnvio').textContent = dataCriacao.toLocaleDateString('pt-BR');
                
                const nomeCliente = osData.veiculo?.cliente?.nome || osData.cliente_nome || 'Cliente não informado';
                const telCliente = osData.veiculo?.cliente?.telefone || osData.cliente_telefone || 'Sem telefone';
                
                document.getElementById('modalCliente').textContent = nomeCliente;
                document.getElementById('modalTelefone').textContent = telCliente;
            }
        } catch (osError) {
        ...
        lista.innerHTML = '';
        itens.forEach(item => {
            const statusColor = item.status_aprovacao === 'aprovado' ? 'color: green;' : 
                              (item.status_aprovacao === 'reprovado' ? 'color: red;' : 'color: inherit;');

            const li = document.createElement('li');
            li.className = 'item-aprovacao-row';
            li.innerHTML = `
                <span style="${statusColor}">
                    ${item.status_aprovacao === 'aprovado' ? '<i class="fas fa-check-circle"></i>' : 
                     item.status_aprovacao === 'reprovado' ? '<i class="fas fa-times-circle"></i>' : ''} 
